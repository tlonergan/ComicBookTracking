...
namespace ComicBook.Models
{
    ...
    {
        public static DateTime NextWednesday(this DateTime startDate)
        {
            IEnumerable<int> dayRange = Enumerable.Range(0, 7);
            foreach (int day in dayRange)
            {
                DateTime testDate = startDate.AddDays(day);
                if (testDate.DayOfWeek == DayOfWeek.Wednesday)
                    return testDate;
            }

            throw new Exception("There are no more wednesdays... Apocolypse!");
        }

        public static DateTime PreviousWednesday(this DateTime startDate)
        {
            IEnumerable<int> dayRange = Enumerable.Range(0, 7);
            foreach (int day in dayRange)
            {
                DateTime testDate = startDate.AddDays(day*-1);
                if (testDate.DayOfWeek == DayOfWeek.Wednesday)
                    return testDate;
            }

            throw new Exception("There were no wednesdays... Apocolypse!");
        }
